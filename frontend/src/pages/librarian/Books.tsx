    ...
    <div className="mx-auto max-w-7xl px-6 py-8 space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-ink-500 mt-1">{BOOKS.length} titles in catalog</p>
        </div>
      ...
      <div className="card overflow-hidden">
          ...
          <tbody className="divide-y divide-ink-100">
            {BOOKS.map((b) => (
              <tr key={b.id} className="hover:bg-parchment-50">
                ...
                <td className="px-4 py-3 text-ink-600">{b.language.toUpperCase()}</td>
                <td className="px-4 py-3 text-ink-600">{b.year}</td>
                <td className="px-4 py-3">
                  <span
                    className={
                      b.available === 0 ? 'chip-danger' : b.available < b.total ? 'chip-warn' : 'chip-success'
                    }
                  >
                    {b.available}/{b.total}
                  </span>
                </td>
                <td className="px-4 py-3 text-ink-700">★ {b.rating.toFixed(1)}</td>
        ...
        <div className="flex items-center justify-between p-4 text-xs text-ink-500 border-t border-ink-100">
          <div>Showing 1–{BOOKS.length} of {BOOKS.length}</div>
          <div className="flex gap-1">
            <button className="btn-ghost text-xs" disabled>← Prev</button>
